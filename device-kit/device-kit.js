...
(function (global) {
  ...
  function dragScroll(el, horizontal = false) {
    let down = false, capturing = false, startPos = 0, startScroll = 0, moved = false, pid = null;
    ...
    el.addEventListener('pointermove', (e) => {
      if (!down) return;
      const d = (horizontal ? e.clientX : e.clientY) - startPos;
      if (!capturing) {
        if (Math.abs(d) <= DRAG_THRESHOLD) return; // still a tap, not a drag
        capturing = true; moved = true;
        try { el.setPointerCapture(pid); } catch (_) {}
        el.style.cursor = 'grabbing';
      }
      if (horizontal) el.scrollLeft = startScroll - d;
